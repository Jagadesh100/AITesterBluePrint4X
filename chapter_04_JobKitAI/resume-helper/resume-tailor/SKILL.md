...
**Resume** — usually a PDF or .docx in the project's `Resume/` folder. If its content is not
...
Write a JSON spec and render it. Point the spec's `output` field into the
project's `output/` folder, naming the file with the target company in the
suffix — `output/<Name>_<Role>_<Company>.docx` — so generated documents never
mix with source files and each tailored copy is identifiable at a glance:
